...
var Graphs = function(cVis) {
    ...
    this.build = function(dateSum) {

        Gschools = new Array();

		// get selected data2 type
		dataType = $("#GroupValue").val();
		
            for(var sch in dateSum)
				if (dateSum[sch]["Date"] == dataType) {
				    Gschools.push(dateSum[sch]);
				}
		
		// set data2 up for graph creater
		for( sch in Gschools)
		{
			//if (Gschools[sch]["Date"] == dataType) {
				Gschools[sch].Data = parseFloat(Gschools[sch]["Duration"]);
			// } else {
			// 		Gschools[sch].Data =  0;
			// }
	...
	// data2 = this is what is going to be displayed. each element needs the attributes .INSTNM and .Data
